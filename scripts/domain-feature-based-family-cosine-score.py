...
#################################################################################################################################################
def execute_pfamscan_on_family_fasta(family_fasta_dirName, pfamscanout_dirName):
	for family_fasta_file in os.listdir(family_fasta_dirName):
		print "Executing pfam_scan on file {0}".format(family_fasta_file)
		run_pfamscan=subprocess.Popen(["./Pfam/pfam_scan.pl", "-outfile", pfamscanout_dirName+"/"+family_fasta_file+".pfamdomtblout", "-e_seq","1e-5", "-e_dom","1e-5", "-fasta",family_fasta_dirName+"/"+family_fasta_file , "-dir","Pfam/database/"])
...
def read_pfamscanout_file(pfamscanout_fileName, pfamscanout_dirName):
	...
	for line in pfamscanout_file:
		...
		if(seqid_startcoord_domain_score_dict.has_key(seqid)):
...
def process_sequence_pairs(seqid_startcoord_domain_score_dict, pfamscanout_fileName, output_file):
	seqid_arr = seqid_startcoord_domain_score_dict.keys()
...
def get_domain_features(startcoord_domain_score_dict_seq_sorted):
	...
	for entry in startcoord_domain_score_dict_seq_sorted:
		domain_id=entry[1].keys()[0]
		domain_score=entry[1].values()[0]
		if(feature_dict.has_key(domain_id)):
...
def get_feature_ids(seq1_domain_id_domain_score_dict, seq2_domain_id_domain_score_dict):
	feature_id_arr=seq1_domain_id_domain_score_dict.keys() + seq2_domain_id_domain_score_dict.keys()
...
def get_feature_vector(seq_domain_id_domain_score_dict, feature_id_arr):
	...
	for feature_id in feature_id_arr:
		if(seq_domain_id_domain_score_dict.has_key(feature_id)):
...
def run_workflow(output_dirName, dataset_name, family_fasta_dirName):
	...
	execute_pfamscan_on_family_fasta(family_fasta_dirName, pfamscanout_dirName)

	output_file=open(output_dirName+"/"+dataset_name+".domain_cosine_score", "w")
...
run_workflow(output_dirName, dataset_name, family_fasta_dirName)
